 ...
 * can tell "too big" from "wrong shape" from "not an image" from "no usable frames" — collapsing
...
sealed interface SpriteSheetFailure {
    ...
    /** `width % height != 0` (or `height <= 0`) — a remainder, never truncated. */
    ...
    /** Every cell is fully transparent after the trailing-transparent-cell clamp: zero usable frames. */
    data object EmptySheet : SpriteSheetFailure
}
