...
fun CharacterImportRejection.messageResource(): Pair<Int, List<Any>> = when (this) {
    CharacterImportRejection.NotPng ->
        R.string.import_rejection_not_png to emptyList()
    is CharacterImportRejection.TooLarge ->
        R.string.import_rejection_too_large to listOf(actualBytes, maxBytes)
    is CharacterImportRejection.Oversized ->
        R.string.import_rejection_oversized to listOf(widthPx, heightPx, maxPx)
    is CharacterImportRejection.NotDivisible ->
        R.string.import_rejection_not_divisible to listOf(widthPx, heightPx)
    CharacterImportRejection.Undecodable ->
        R.string.import_rejection_undecodable to emptyList()
    CharacterImportRejection.EmptySheet ->
        R.string.import_rejection_empty_sheet to emptyList()
    is CharacterImportRejection.CapReached ->
        R.string.import_rejection_cap_reached to listOf(cap)
}
