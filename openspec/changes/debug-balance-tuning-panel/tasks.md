...
- [x] 1.1 RED: write `TuningRowStateTest` (`app/src/test/java/.../debug/tuning/`) — the row-state
...
- [x] 1.2 GREEN: create `app/src/debug/java/.../debug/tuning/TuningRowState.kt` — `Staleness` (`Fresh`,
...
- [x] 1.3 RED: write `TuningRegistryCoverageTest` — `tuningRowOf` over `BalanceConfig.ALL` (5) and
...
- [x] 1.4 GREEN: confirm 1.3 passes against 1.2's `tuningRowOf`; no production change expected unless
      the registry walk reveals a gap.
- [x] 1.5 RED: write `TuningRejectionCopyTest` — `parseTypedValue(field, text)` returns `Unparseable`
...
- [x] 1.6 GREEN: create `app/src/debug/java/.../debug/tuning/TuningRejectionCopy.kt` — pure
      `ParsedInput<T>` (`Valid<T>`, `Unparseable`), `parseTypedValue`, `rejectionMessage`,
      `unparseableMessage`. No Android import. (Design decision 4.)
- [x] 1.7 GREEN: create `app/src/debug/java/.../debug/tuning/TuningPanelViewModel.kt` —
...
- [x] 1.8 GREEN: create `app/src/debug/java/.../debug/tuning/TuningPanelScreen.kt` — Compose panel
...
- [x] 1.9 GREEN: create `app/src/debug/java/.../debug/tuning/TuningPanelActivity.kt` —
      ...
      half.)
- [x] 1.10 GREEN: create `app/src/debug/res/values/strings.xml` — the marker label string, value equal
      to `TUNING_PANEL_MARKER` (token T2), used as the debug launcher activity's `android:label`.
- [x] 1.11 GREEN: modify `app/src/debug/AndroidManifest.xml` — add a second `MAIN`/`LAUNCHER`
...
- [x] 1.12 RED: write `TuningPanelNoLeakCodeTest` (`app/src/test/java/.../debug/tuning/`,
...
- [x] 1.13 GREEN: confirm 1.12 passes against the current tree; no production change expected — this
      is a structural guard against a future leak, not a fix for one that exists today.
- [x] 1.14 RED: write `TuningPanelNoDebugFlagCodeTest` (same source-scan shape) — `BuildConfig.DEBUG`
...
- [x] 1.15 GREEN: confirm 1.14 passes against the current tree; no production change expected.
- [x] 1.16 GREEN: modify `.github/workflows/ci.yml` — append `assembleRelease` to the existing Gradle
...
- [x] 1.17 RED: write `TuningPanelMarkerTest` (`app/src/test/java/.../debug/tuning/`) —
      ...
      1b.)
- [x] 1.18 GREEN: confirm 1.17 passes — it requires 1.2's marker const, 1.9's activity package, and
      1.16's CI edit to all already exist; no further production change expected.
- [x] 1.19 Run `./gradlew assembleDebug testDebugUnitTest :core:domain:test assembleDebugAndroidTest
