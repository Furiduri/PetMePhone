...
1. **Build a disposable spike harness in `:feature:overlay`.**
...
2. **Register a runtime `BroadcastReceiver` for `ACTION_SCREEN_ON`/`ACTION_SCREEN_OFF` in the spike harness.**
...
3. **Instrument `Choreographer` frame health while the screen is off.**
...
4. **Run the spike on `emulator-5554` with the screen off for ≥10 minutes.**
...
5. **Analyze the log and answer the three spike questions explicitly.**
...
6. **Determine which PR 2 design elements the findings force, per `design.md`'s decision fork.**
...
7. **Write the findings back to issue #36 as a comment.**
...
8. **Append the findings to `design.md` under "PR 0 — the spike, precisely".**
...
9. **Remove the spike harness (Tasks 1–3) from the overlay module.**
...
11. **Create `PetSpriteRow.kt`.** `[SHEET-2]`
...
12. **Create `SpriteSheetFailure.kt`.** `[SHEET-7]`
...
13. **Create `SpriteGrid.kt` with `SpriteGridResult`.** `[SHEET-3]` `[SHEET-4]`
...
14. **Create `SpriteLayout.kt`.** `[SHEET-3]`
...
15. **Unit test: `SpriteGrid.of` divisibility and oversize rules.** `[SHEET-4]` `[SHEET-5]`
...
16. **Unit test: `SpriteLayout` cell arithmetic and frame-count invariant.** `[SHEET-2]` `[SHEET-3]`
...
17. **Confirm `:core:domain` stays Android-free.** `[SHEET-1]`
...
18. **Create `SpriteSheetResult.kt`.** `[SHEET-7]`
...
19. **Create `SpriteSheetDecoder.kt`: header-first read.** `[SHEET-5]`
...
20. **Extend the decoder: full decode to `ARGB_8888`, never `HARDWARE`.** `[SHEET-6]`
...
21. **Create `TransparentCellScanner.kt`.** `[SHEET-8]`
...
22. **Unit test (Robolectric, `@Config(sdk = [36])`): header-first ordering never reaches full decode.** `[SHEET-5]`
...
23. **Unit test (Robolectric): corrupt bytes, config assertion, transparent-cell clamp.** `[SHEET-6]` `[SHEET-8]` `[SHEET-9]`
...
24. **Add binary PNG fixtures for the above tests.** `[SHEET-5]` `[SHEET-6]` `[SHEET-8]`
...
27. **Create `ScreenStateMonitor.kt`.** `[RENDER-4]`
...
28. **Create `PetAnimationConfig` and wire the frame interval as an injected value.** `[RENDER-3]`
...
29. **Create `PetOverlayStateHolder.kt`.** `[SHEET-9]` `[RENDER-1]`
...
30. **Create `PetOverlay.kt`: IDLE frame drawing.** `[RENDER-1]` `[RENDER-2]`
...
31. **Add the manual interval clock with screen-off suspension.** `[RENDER-3]` `[RENDER-4]`
...
32. **Draw the broken-placeholder shape on decode failure.** `[RENDER-5]` `[RENDER-6]`
...
33. **Wire `PetOverlayService.kt`: replace the magenta placeholder.** `[RENDER-7]`
...
34. **Add the `@Ignore`d `ProgrammerArtGenerator` JUnit test.**
...
35. **Run `ProgrammerArtGenerator` manually and commit the output asset.**
...
36. **Bind the decoder, config, and monitor in `OverlayModule.kt`.**
...
37. **Unit test (`runTest` virtual time): clock advance, suspend, resume.** `[RENDER-3]` `[RENDER-4]`
